...
<script type="text/javascript">
	$(function() {
		$('#admin_zygl_treegrid').treegrid({
			url : '${pageContext.request.contextPath}/do/resource/treegrid',
			idField : 'id',
			treeField : 'text',
			parentField : 'pid',
			fit : true,
			...
			columns : [ [ {
				...
				width : 50,
				formatter : function(value, row, index) {
					var str = '<span onclick="admin_zygl_editFun(\'{0}\');" class="icon-edit icon-block"></span>&nbsp;<span onclick="admin_zygl_deleteFun(\'{1}\');"  class="icon-no icon-block"></span>';
	...
	function admin_zygl_appendFun() {
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/ht/admin/zyglAdd.jsp',
			width : 500,
			height : 350,
			modal : true,
			title : '资源添加',
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_zyglAdd_addForm').form('submit', {
						url : '${pageContext.request.contextPath}/do/resource/add',
	...
	function admin_zygl_editFun(id) {
		...
		var node = $('#admin_zygl_treegrid').treegrid('getSelected');
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/ht/admin/zyglEdit.jsp',
			width : 500,
			height : 350,
			modal : true,
			title : '资源编辑',
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_zyglEdit_editForm').form('submit', {
						url : '${pageContext.request.contextPath}/do/resource/edit',
						success : function(result) {
							try {
								var r = $.parseJSON(result);
								if (r.success) {
	...
	function admin_zygl_deleteFun(id) {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/do/resource/remove',
						data : {
							id : node.id
						},
						cache : false,

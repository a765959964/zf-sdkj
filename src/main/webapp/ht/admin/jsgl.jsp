...
<script type="text/javascript">
	$(function() {
		$('#admin_jsgl_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/do/role/datagrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function admin_jsgl_editFun(id) {
		...
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/ht/admin/jsglEdit.jsp',
			width : 500,
			height : 200,
			modal : true,
			title : '编辑角色',
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_jsglEdit_editForm').form('submit', {
						url : '${pageContext.request.contextPath}/do/role/edit',
	...
	function admin_jsgl_appendFun() {
		$('#admin_jsgl_datagrid').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/ht/admin/jsglAdd.jsp',
			width : 500,
			height : 200,
			modal : true,
			title : '添加角色',
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_jsglAdd_addForm').form('submit', {
						url : '${pageContext.request.contextPath}/do/role/add',
						success : function(result) {
							try {
								var r = $.parseJSON(result);
								if (r.success) {
		...
		if (rows.length > 0) {
			$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/do/role/remove',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
